#!/bin/bash
echo "🔒 启动电商AI Agents权限控制服务器..."
cd /opt/ecommerce-ai-agents

# 停止旧服务器
pkill -f "simple-login-server.js" 2>/dev/null
pkill -f "server-login-simple.js" 2>/dev/null
sleep 2

# 创建简单的权限控制服务器
cat > bin/secure-server.js << 'SERVER_EOF'
...
# 启动服务器
nohup node bin/secure-server.js > logs/secure-server.log 2>&1 &
SERVER_PID=$!
sleep 3
...
echo "🧪 测试权限控制:"
echo "1. 测试未登录访问管理后台:"
curl -s -I "http://localhost:3002/admin" | head -1
echo ""
echo "2. 测试健康检查（公开）:"
curl -s "http://localhost:3002/health" | python3 -c "
import sys, json
try:
    data = json.load(sys.stdin)
    print('✅ 健康检查正常:', data['status'])
    print('✅ 安全状态:', data['security'])
except:
    print('❌ 测试失败')
"
echo ""
...
curl -s -X POST "http://localhost:3002/api/auth/login" \
  ...
  -d '{"email":"admin@ecommerce-ai-agents.org","password":"Admin123!"}' | python3 -c "
import sys, json
try:
    data = json.load(sys.stdin)
    if data['success']:
        print('✅ 登录成功:', data['data']['user']['role'])
    else:
...
echo "   ✅ 未登录用户无法访问产品页面"
echo "   ✅ 非管理员无法访问管理后台"
echo "   ✅ 只有登录用户才能访问受保护页面"
echo ""
echo "📝 日志文件: logs/secure-server.log"
