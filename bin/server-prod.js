...
const server = http.createServer(async (req, res) => {
    ...
    try {
        ...
        if (pathname === '/health') {
            ...
            log('info', `健康检查: ${req.socket.remoteAddress}`);
            ...
            res.writeHead(200);
            res.end(JSON.stringify(response));
            log('info', `AI对话: "${message.substring(0, 30)}..."`);
...
<body>
    ...
    <div class="table">
        ...
        <table>
            ...
            <tr><td>数据库连接</td><td>SQLite</td><td><span style="color:green">✅ 正常</span></td></tr>
    ...
    <div class="table">
        <h2>👥 用户管理</h2>
        <table>
            <tr><th>邮箱</th><th>角色</th><th>操作</th></tr>
            ${productionData.users.map(u => `
                <tr>
                    <td>${u.email}</td>
                    <td>${u.role}</td>
                    <td>
...
server.listen(PORT, HOST, () => {
    ...
    console.log(`📊 功能模块: ${Object.keys(config.features || {}).filter(k => config.features[k]).join(', ')}`);
    console.log('==========================================');
    console.log('👤 默认账户:');
